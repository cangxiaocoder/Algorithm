...
public class Offer12 {
    ...
    private boolean dfs2(char[][] board, int i, int j, char[] chars, int index, boolean[][] visited) {
        //word单词全部搜索比对完成
        if (i < 0 || i >= board.length || j < 0 || j >= board[0].length || chars[index] != board[i][j] || visited[i][j]) {
            return false;
        }
        ...
        for (int[] direction : directions) {
            ...
            result = dfs2(board, newI, newJ, chars, index + 1, visited);
            if (result) {
                break;
            }
        }

        //若某条路径不通，则将此位置重新设为false，如某个节点从当前节点出发是访问过的，但是从另外一个节点出发是未访问过的。所以需要重设为false,
        visited[i][j] = false;
        return result;
    }
